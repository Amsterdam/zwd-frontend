...
export const useApiCache = () => {
  ...
  const getCacheItem = useCallback(
    (key: string): ApiCacheItem | undefined => cache[key],
    [cache]
  )
  const setCacheItem = useCallback(
    (key: string, value: any) => dispatch({ type: "SET_ITEM", key, value }),
    [dispatch]
  )
  const updateCacheItem = useCallback(
    (key: string, updater: (cache: any) => void) =>
      dispatch({ type: "UPDATE_ITEM", key, updater }),
    [dispatch]
  )
  const addErrorToCacheItem = useCallback(
    (key: string, error: any) => dispatch({ type: "ADD_ERROR", key, error }),
    [dispatch]
  )
